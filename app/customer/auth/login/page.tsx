...
import { useIsNativeApp } from "@/lib/capacitor-photo"
    ...
    <div className="flex min-h-screen">
      <div className="w-full max-w-md md:w-1/2 mx-auto bg-background px-6 py-12 sm:px-10">
        <div className="w-full">
          ...
          {/* OAuth — disabled in the native app shell: Google/Facebook block
              plain embedded WebViews, and the ASWebAuthenticationSession +
              deep-link bridge needed to do this properly is separate scope. */}
          <div className="grid gap-3 sm:grid-cols-2">
            <Button variant="outline" type="button" disabled={isNative}
              onClick={() => { if (!isNative) window.location.href = "/api/customer/auth/oauth/google" }}>
              ...
              {isNative ? "Google (coming soon)" : "Google"}
            </Button>
            <Button variant="outline" type="button" disabled={isNative}
              onClick={() => { if (!isNative) window.location.href = "/api/customer/auth/oauth/facebook" }}>
              <svg className="mr-2 h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
                <path d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"/>
              </svg>
              {isNative ? "Facebook (coming soon)" : "Facebook"}
            </Button>
          </div>
          {isNative && (
            <p className="mt-2 text-center text-xs text-muted-foreground">
              Google/Facebook sign-in is coming soon to the app — use phone or email for now.
            </p>
          )}
