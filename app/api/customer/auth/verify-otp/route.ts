...
export async function POST(req: NextRequest) {
  try {
    ...
    // Both OTPs are valid - update user
    await query(
      `UPDATE users
       SET email_verified = true,
           ...
           metadata = metadata - 'email_otp' - 'email_otp_expires_at' - 'phone_otp' - 'phone_otp_expires_at'
       WHERE id = $1`,
      [user.id]
    )
    ...
    // Create session and set cookies (only for non-pending users)
    if (!requiresApproval) {
